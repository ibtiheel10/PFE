  ...
  <div class="jp-root">
    ...
    <header class="jp-nav">
      <div class="jp-nav-inner">
        <!-- Logo -->
        <router-link :to="userRole === 'entreprise' ? '/dashboard-entreprise' : '/candidat/jobs'" class="jp-brand">
        ...
        <!-- Breadcrumb -->
        <nav class="jp-crumb">
          <router-link :to="userRole === 'entreprise' ? '/dashboard-entreprise' : '/candidat/jobs'" class="jp-crumb-link">
            {{ userRole === 'entreprise' ? 'Tableau de bord' : 'Offres d\'emploi' }}
          </router-link>
    ...
    <main class="jp-main" v-if="job">
      ...
      <section class="jp-hero">
        <div class="jp-hero-inner">
          ...
          <div class="jp-apply-panel">
            <!-- ENTERPRISE VIEW -->
            <template v-if="userRole === 'entreprise'">
              <div class="jp-ent-action-box">
                <p class="jp-ent-label">Gestion du poste</p>
                <div class="jp-ent-btns">
                  <button class="jp-btn-manage" @click="goToQCM">
      ...
      <div class="jp-grid">
        ...
        <div class="jp-right">
          ...
          <!-- Candidates list (Enterprise only) -->
          <div class="jp-card" v-if="userRole === 'entreprise'">
             <div class="jp-card-head">
               <div class="jp-card-ico jp-ico-amber"><i class="fa-solid fa-users"></i></div>
               <h3 class="jp-card-title">Candidats inscrits</h3>
             </div>
             <div class="jp-card-body jp-candidates-mini-list">
               <div v-for="c in job.candidatures" :key="c.id" class="jp-cand-item">
                 <img :src="`https://ui-avatars.com/api/?name=${encodeURIComponent(c.candidat.nom)}&background=random`" class="jp-cand-avatar">
                 <div class="jp-cand-info">
                   <div class="jp-cand-name">{{ c.candidat?.nom }} {{ c.candidat?.prenom }}</div>
                   <div class="jp-cand-date">{{ new Date(c.DateCandidature).toLocaleDateString() }}</div>
                 </div>
                 <div class="jp-cand-tag" :class="getStatusClass(c.statut)">{{ c.statut }}</div>
               </div>
               <div v-if="!job.candidatures || job.candidatures.length === 0" class="jp-cand-empty">
          ...
          <!-- Tips (Candidate only) -->
          <div class="jp-card jp-tips-card" v-if="userRole !== 'entreprise'">
...
const userRole = computed(() => localStorage.getItem('userRole') || 'candidat');

const job           = ref<OffreEmploi | any>(undefined);
...
const fetchMyApplications = async () => {
  if (userRole.value !== 'candidat') return;
  try {
...
const goToQCM = () => {
  if (job.value) router.push(`/job-qcm/${job.value.id}`);
};
</script>
