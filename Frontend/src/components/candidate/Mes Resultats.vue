...
onMounted(async () => {
  try {
      ...
      if (resStats.progression && resStats.progression.length > 0) {
          const dates = resStats.progression.map((p: any) => new Date(p.date).toLocaleDateString('fr-FR', { day: '2-digit', month: 'short' }));
          const scores = resStats.progression.map((p: any) => p.score);
      ...
      if (err.response?.status === 403) {
          import('../../services/swal').then(module => { const Swal = module.default;
              Swal.fire({ title: 'Accès refusé', text: "L'évaluation n'est pas encore terminée ou le score n'est pas finalisé.", icon: 'error' });
          });
