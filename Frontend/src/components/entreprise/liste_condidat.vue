        ...
        <div class="filters-card">
            ...
            <div class="filter-group">
                    ...
                    <select v-model="sortField" class="filter-select-modern">
                        ...
                        <option value="role">Trier par Poste</option>
                    </select>
                    <svg class="select-arrow" width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="6 9 12 15 18 9"></polyline></svg>
                </div>
        ...
        <div class="card table-card">
            <div class="table-responsive">
                <table class="modern-table">
                    ...
                    <tbody>
                        <tr v-for="candidate in paginatedCandidates" :key="candidate.id" :class="{'row-selected': isSelected(candidate)}" @click="viewCandidateDetail(candidate)" style="cursor: pointer;">
...
const fetchCandidats = async () => {
    try {
        ...
        // Map backend data to frontend field names if necessary
        candidatesList.value = res.data.map((c: any) => ({
            ...c,
            name: c.name || [c.nom, c.prenom].filter(Boolean).join(' ') || 'Candidat inconnu',
            role: c.role || c.offre?.titre || 'Candidat'
        }));
    } catch (e: any) {
...
const filteredCandidates = computed(() => {
    ...
    return list.slice().sort((a: any, b: any) => {
        ...
        if (typeof valA === 'string' && typeof valB === 'string') {
            return valA.localeCompare(valB, 'fr', { sensitivity: 'base' }) * modifier;
        }
...
const getScoreColor = (candidate: any) => {
    ...
    if (statut === 'Accepté') return 'green-fill';
    if (statut === 'Entretien') return 'blue-fill';
    if (statut === 'Refusé') return 'red-fill';
...
const getScoreTextClass = (candidate: any) => {
    ...
    if (statut === 'Accepté') return 'text-green';
    if (statut === 'Entretien') return 'text-blue';
    if (statut === 'Refusé') return 'text-red';
    return 'text-orange';
};
...
const getScoreLabel = (candidate: any) => {
    ...
    if (statut === 'Accepté') return 'Accepté';
...
const sendContact = async () => {
    ...
    try {
        ...
        // Mettre à jour le statut du candidat localement pour afficher "Entretien" via UI
        const candidateInList = candidatesList.value.find(c => c.id === contactTarget.value.id);
        if (candidateInList) {
            candidateInList.statut = 'Entretien';
            // Optional: You could update backend here if needed, but assuming email implies interview:
            await api.put(`/Entreprise/candidatures/${contactTarget.value.id}/statut`, {
                 nouveauStatut: 'Entretien'
            }).catch(e => console.error("Erreur mise à jour statut", e));
        }

        showContactModal.value = false;
        Swal.fire({ title: 'Message envoyé', text: `Votre message a été envoyé à ${contactTarget.value.name}. Le statut est passé à Entretien.`, icon: 'success' });
    } catch (e: any) {
...
const viewCandidateDetail = (candidate: any) => {
