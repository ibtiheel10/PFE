        ...
        <section class="form-card" v-show="step === 1">
          ...
          <div class="field-row">
            <div class="field">
              ...
              <CategoryDropdown v-model="form.category" placeholder="Sélectionner une catégorie" />
            </div>
            <div class="field">
        ...
        <section class="form-card" v-show="step === 3">
          ...
          <div class="qcm-box">
            ...
            <div class="field-row" style="margin-top:16px">
              ...
              <div class="field">
                <label>Seuil minimal d'acceptation (%)</label>
                <input v-model.number="form.mcqPassScore" type="number" min="0" max="100" placeholder="70" />
                <span class="field-hint">Score minimum pour être Accepté</span>
              </div>
            </div>
...
const submitPost = async () => {
  ...
  try {
     ...
     const payload = {
        ...
        seuilMinimal: form.value.mcqPassScore,
...
/* ── Header ── */
.ap-header {
  background: linear-gradient(135deg, #0f172a 0%, #1e40af 60%, #1e40af 100%);
  padding: 24px 40px 0;
  position: relative;
  overflow: hidden;
}
...
.btn-next:hover { transform: translateY(-1px); box-shadow: 0 6px 20px rgba(30, 64, 175, 0.4); }
.btn-back-step {
  background: #f8fafc;
  border: 1.5px solid #e2e8f0;
  color: #64748b;
