<template>
  <div class="content-container animate-fade-in">
    <div class="page-header">
        ...
        <button class="new-post-btn" style="width: auto; padding: 0.5rem 1rem; background: #1e40af; border-radius: 10px;" @click="createNewPost">
          ...
          <form id="create-post-form" @submit.prevent="submitPost">
            ...
            <div v-show="currentTab === 'settings'" class="tab-content">
              ...
              <div class="form-row">
                ...
                <div class="form-group">
                  <label for="mcqPassScore">Seuil minimal d'acceptation (%)</label>
                  <input 
                    type="number" 
                    id="mcqPassScore" 
                    v-model.number="formData.mcqPassScore" 
                    min="0"
                    max="100"
                    placeholder="70"
                  >
                  <p class="help-text" style="font-size: 11px; margin-top: 4px;">Score minimum pour être Accepté</p>
                </div>
              </div>
        ...
        <div class="qcm-panel">
          ...
          <div class="qcm-panel-body">
            ...
            <div class="qcm-desc-box">
              <p class="qcm-desc-label" style="text-transform: uppercase; font-weight: bold; font-size: 0.85rem; color: #1e40af; letter-spacing: 0.5px; margin-bottom: 8px;">DESCRIPTION DU POSTE</p>
              ...
              <div style="margin-top: 16px; padding-top: 16px; border-top: 1px dashed #e2e8f0;">
                <p class="qcm-desc-label" style="text-transform: uppercase; font-weight: bold; font-size: 0.85rem; color: #1e40af; letter-spacing: 0.5px; margin-bottom: 8px;">COMPÉTENCES REQUISES</p>
...
import CategoryDropdown from '@/components/composants_reutilisables/CategoryDropdown.vue'
...
const editJob = (job: any) => {
    ...
    formData.value = {
      ...
      mcqPassScore: raw.seuilMinimal || 70
    };
    ...
    try {
        ...
        const buildPayload = () => {
            const raw: any = {
                ...
                seuilMinimal: formData.value.mcqPassScore || 70,
...
.tab-indicator {
    ...
    top: 4px;
    bottom: 4px;
    background: linear-gradient(135deg, #1e3a8a 0%, #1e40af 100%); /* New Blue Gradient */
    border-radius: 8px;
    transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1);
    z-index: 1;
...
select {
    ...
    color: #111827;
    background: #FAFBFC;
    background-image: url("data:image/svg+xml,%3Csvg width='14' height='9' viewBox='0 0 14 9' fill='none' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M1 1.5L7 7.5L13 1.5' stroke='%231e40af' stroke-width='2.5' stroke-linecap='round' stroke-linejoin='round'/%3E%3C/svg%3E");
...
select:hover {
    ...
    border-color: #1e40af;
    box-shadow: 0 3px 10px rgba(30, 64, 175, 0.12), 0 1px 3px rgba(0, 0, 0, 0.05);
...
.modal-btn-confirm:hover { background: #1D4ED8; box-shadow: 0 4px 12px rgba(30, 64, 175, 0.2); }
...
.qcm-panel-header::before {
    ...
    height: 3px;
    background: linear-gradient(90deg, #1e3a8a, #1e40af, #06B6D4, #8B5CF6, #1e3a8a);
...
.qcm-question-card::before {
    ...
    top: 0; left: 0; right: 0;
    height: 2px;
    background: linear-gradient(90deg, #1e3a8a, #1e40af, #06B6D4);
    opacity: 0;
...
.qcm-correct-label:hover { border-color: #1e3a8a; background: #EEF2FF; transform: scale(1.1); }
