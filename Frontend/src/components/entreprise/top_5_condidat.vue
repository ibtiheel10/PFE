                        ...
                        <td>
                            <div class="candidate-profile">
                                <div class="avatar-wrapper" style="width: 36px; height: 36px;">
                                    <img v-if="candidate.avatar" :src="candidate.avatar" class="c-avatar-lg" :alt="candidate.name">
                                    <div v-else class="c-avatar-lg bg-[#0f172a] flex items-center justify-center">
                                        <LogoIcon customClass="w-5 h-5 text-white opacity-80" />
                                    </div>
                                ...
                                <div class="candidate-details">
                                    ...
                                    <div class="text-[0.7rem] text-gray-400 font-medium">{{ candidate.email }}</div>
                                </div>
                            </div>
                        </td>
...
const getScoreColor = (candidate: any) => {
    ...
    if (s === 'Accepté') return 'green-fill';
    if (s === 'Entretien') return 'blue-fill';
...
const getScoreTextClass = (candidate: any) => {
    const s = getDisplayStatus(candidate);
    if (s === 'Accepté') return 'text-green';
    if (s === 'Entretien') return 'text-blue';
...
const getScoreLabel = (candidate: any) => {
    const s = getDisplayStatus(candidate);
    if (s === 'Refusé') return 'Insuffisant';
...
const getDisplayStatus = (candidate: any): string => {
    return candidate.statut || 'En attente';
};

const getStatusClass = (statutText: string): string => {
    const s = (statutText || '').toLowerCase();
    if (s === 'entretien') return 'entretien'; // Bleu (mis à jour en CSS)
    if (s === 'accepté' || s === 'accepte') return 'accepted'; // Vert
    if (s === 'refusé' || s === 'refuse') return 'refused'; // Rouge
