...
const handleLogin = async () => {
  ...
  try {
    ...
    // Si le backend retourne un token directement (Admin — pas d'OTP)
    if ((response as any).token) {
      const authData = response as any;
      // Sauvegarder la session admin
      localStorage.setItem('userToken', authData.token);
      localStorage.setItem('userRole', authData.role.toLowerCase());
    ...
    // Sinon, le backend a envoyé l'OTP → on ouvre le modal de vérification
